...
\author{Omar Alam, Nirmal Chaudhari, Kevin Zhu \\
  ...
  Code for project available: \href{https://github.com/alamo3/SFWRENG-4AL3-Group-27}{GitHub}
  }
...
. 
\subsection{Multi-layered Perceptron Hybrid}

Another method tried was using a Multi-layered Perceptron (MLP). MLPs are not efficient or designed for image processing as that utilize many fully connected layers. As we still wanted to have another method to compare to, we decided to implement the MLP after extracting features from images. We did this by using multiple layers of convolution to extract features and scale each pixel to include nearby features, giving each pixel 128 features to train the MLP on.
...
self.cnn = nn.Sequential(
    ...
    nn.MaxPool2d(2))
\end{lstlisting}
The model then feeds each pixel and related features into a fully connected hidden layer, and uses that to calculate the estimated depth. 
...
datapoints. 
\subsection*{MLP Hybrid Evaluation}
As our MLP Hybrid hides lots of data due to efficiency constraints, to first gauge the effectiveness by training the dataset on the same 1000 images as the Initial Implementation. The results are the results of training the MLP model for 8 epochs. The model was stopped at 8 due to poor training time and the performance metrics not improving. The performance was also lacking even though it is comparable to the previous ResNet implementations. 
...
\textbf{Metric} & \textbf{Value} \\
\hline
Validation Loss (SiLog) & 0.2551 \\
Training Loss (SiLog) & 0.5040 \\
Rel Sq & 3.8085 \\
Abs Rel & 0.3608 \\
iRMSE & 0.0397 \\
\hline
...
\end{tabular}
\caption{MLP Model Evaluation Metrics on Validation Set after 8 Epochs}
\label{tab:metrics}
\end{table}
...
The results of the baseline show that our initial ResNet attempts, while better than a random depth map are not good models as they are comparable to the zero baseline. However, the vision transformer model is not only better than our initial attempts it is also much better than the baselines, meaning that the model is properly evaluating depth as compared to the previous models. 
